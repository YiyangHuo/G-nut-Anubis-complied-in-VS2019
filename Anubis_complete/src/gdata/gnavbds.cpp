...
// ----------
/*
int t_gnavbds::operator<(const t_gnavbds& data) const
{       
...
// ----------
/*
int t_gnavbds_struct::operator==(const t_gnavbds& data) const
{       
  int _dtot = int( tot.diff( data.tot ) );
...
//  _toc.tsys(t_gtime::GPS);
  ...
  if (data_21 == 0) { //RT BDS broadcast ==0
	  ...
	  sprintf(tmp, "%4i %6i", data_21 + CONV_BWK2GWK, data_11);
  }
...
int t_gnavbds::iod() const
{
	...
	//t_gtime bdsTime = _toc;     // _toc is in UTC
